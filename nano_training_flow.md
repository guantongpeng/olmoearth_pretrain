...
以 `scripts/official/nano.py` 为入口，完整推演从启动到每一步训练迭代的全流程。
...
`main()` 来自 `olmoearth_pretrain.internal.experiment`，负责按顺序调用各 builder 构建配置，然后启动训练。
...
CommonComponents(
    run_name="...",           # 实验名称
    save_folder="...",        # 检查点/日志保存路径
    training_modalities=[     # 9 个训练模态
    ...
    tokenization_config=None,  # 使用默认波段分组
)
```
...
### 2.1 模型尺寸 (`internal/utils.py:83`)

```python
MODEL_SIZE_ARGS["nano"] = {
    "encoder_embedding_size": 128,   # 编码器嵌入维度 D=128
    "encoder_depth": 4,              # 编码器 Transformer 层数
    "encoder_num_heads": 8,          # 编码器注意力头数
    "decoder_embedding_size": 128,   # 解码器嵌入维度
    "decoder_depth": 4,              # 解码器 Transformer 层数
    "decoder_num_heads": 8,          # 解码器注意力头数
    "mlp_ratio": 4.0,               # MLP 隐藏层倍率
}
```

### 2.2 编码器配置 (`nano.py:31`)

```python
EncoderConfig(
    embedding_size=128,              # D = 128
    num_heads=8,                     # 8 个注意力头，每头 128/8=16 维
    depth=4,                         # 4 层 Transformer
    mlp_ratio=4.0,                   # MLP 隐藏层 = 128 * 4 = 512
    supported_modality_names=[上述 9 个模态],
    max_patch_size=8,                # 最大 patch 大小
    drop_path=0.1,                   # DropPath 概率
    max_sequence_length=12,          # 最大时间步数
    use_linear_patch_embed=False,    # 使用 Conv2d (非 Linear) patch embedding
)
```

**编码器内部组件**：
- `MultiModalPatchEmbeddings`: 9 个模态，共 11 个 bandset，每个 bandset 一个 FlexiPatchEmbed
- `CompositeEncodings`: 4 种编码各占 D/4 = 32 维
  - Channel Embedding: 可学习, 11 个 bandset × 32 维
  - Temporal Position: 冻结 sincos, 32 维
  - Month Embedding: 冻结 sincos, 32 维
  - Spatial Embedding: 冻结 sincos+GSD, 32 维
- 4 个 Transformer Block: pre-norm, 支持 Flash Attention
- `ProjectAndAggregate`: 投影+池化用于对比学习

### 2.3 解码器配置 (`nano.py:42`)
...
**解码器内部组件**：
- `encoder_to_decoder_embed`: Linear(128→128) 维度映射
- `mask_token`: 可学习掩码 token (128 维)
- `CompositeEncodings`: 同编码器结构
- 4 个 Transformer Block: 交叉注意力 (Q=decoder tokens, K/V=encoder visible tokens)
- `to_output_embed`: 逐 bandset 输出投影
...
**LatentMIM 模型**：
- `encoder`: 在线编码器 (4 层 Transformer)
- `decoder`: 解码器 (4 层交叉注意力)
...
**损失组成**：
- `ModalityPatchDiscriminationLoss(tau=0.1)`: 逐模态逐样本判别，L2 归一化后余弦相似度 + 交叉熵
...
ContrastiveLatentMIMTrainModuleConfig(
    ...
    ema_decay=(1.0, 1.0),           # EMA 衰减: start=1.0, end=1.0 → 实际不更新目标编码器
    token_exit_cfg={modality: 0 for modality in training_modalities},  # 所有模态 token 退出层=0
...
**EMA 衰减 (1.0, 1.0) 的含义**：目标编码器参数不通过 EMA 更新，保持初始随机权重。这是 Phase 2 的设计选择——目标编码器提供随机投影目标，类似 BYOL 的初始阶段。
...
OlmoEarthDataLoaderConfig(
    num_workers=12,                  # 12 个数据加载工作进程
    global_batch_size=512,           # 全局批次大小
    token_budget=2250,               # 每实例最大 token 数
    prefetch_factor=2,               # 预取因子
    sampled_hw_p_list=[1,2,...,12],  # 空间 patch 数候选 [1, 12]
    min_patch_size=1,                # 最小 patch 大小
    max_patch_size=8,                # 最大 patch 大小
    seed=3622,                       # 随机种子
    num_masked_views=2,              # 双掩码视图 (ContrastiveLatentMIM)
    masking_config=get_masking_config(common),  # 掩码在 worker 中执行
    tokenization_config=None,        # 默认波段分组
)
```
...
```python
max_duration = Duration.epochs(300)             # 训练 300 个 epoch
permanent_save_interval = 5000                  # 每 5000 步永久保存
ephemeral_save_interval = 250                   # 每 250 步临时保存
```
...
步骤 1: 基础随机掩码
  ...
  (encode_ratio=0.5, decode_ratio=0.5, 剩余=0)
...
步骤 3: 选择编码/解码 bandset
  ...
  随机选择 num_to_encode ∈ [min, max] 个用于编码
...
│ │    encoding[..., 0:32]  += channel_embed[modality][bandset]  │ │
│ │    encoding[..., 32:64] += sincos_pos_embed[timestep]        │ │
│ │    encoding[..., 64:96] += month_embed[month]                │ │
│ │    encoding[..., 96:128] += sincos_2d_embed[h,w] * gsd_ratio│ │
│ │    token = token + encoding                                  │ │
│ │                                                              │ │
│ │ 3. Collapse: 拼接所有模态 → [B, total_tokens, 128]           │ │
│ │                                                              │ │
│ │ 4. 移除掩码 token:                                           │ │
...
│ │                                                              │ │
│ │ 5. 4 层 Transformer Block:                                   │ │
│ │    for blk in self.blocks:                                   │ │
│ │      x = x + DropPath(LayerScale(Attention(LayerNorm(x))))   │ │
│ │      x = x + DropPath(LayerScale(MLP(LayerNorm(x))))         │ │
│ │    Attention: 8 heads, 16 dim/head                           │ │
│ │    MLP: 128 → 512 → 128                                     │ │
│ │                                                              │ │
│ │ 6. LayerNorm                                                 │ │
│ │ 7. 恢复掩码位置 (0 填充)                                      │ │
│ │ 8. 拆分回 per-modality: TokensAndMasks                       │ │
│ │ 9. ProjectAndAggregate → pooled [B, 128]                     │ │
│ └──────────────────────────────────────────────────────────────┘ │
│                                                                  │
│ 输出: latent (TokensAndMasks), pooled (Tensor [B, 128])          │
│                                                                  │
│ ┌─ 解码器 Predictor.forward() ────────────────────────────────┐ │
│ │                                                              │ │
│ │ 1. LayerNorm + Linear(128→128) 维度映射                      │ │
│ │ 2. 在 DECODER 位置插入 mask_token (可学习, 128 维)            │ │
│ │ 3. 添加 Composite Encodings (同编码器结构)                    │ │
│ │ 4. 拼接所有模态 → split_x_y():                               │ │
...
│ │ 7. 逐 bandset 输出投影: Linear(128→128) + LayerNorm          │ │
│ └──────────────────────────────────────────────────────────────┘ │
│                                                                  │
...
│ pooled_a = F.normalize(pooled_a, p=2, dim=-1)  # [B, 128]      │
│ pooled_b = F.normalize(pooled_b, p=2, dim=-1)  # [B, 128]      │
...
| 组件 | 嵌入维度 | 层数 | 头数 | MLP 维度 |
|------|---------|------|------|---------|
| 在线编码器 | 128 | 4 | 8 (16/head) | 512 |
| 解码器 | 128 | 4 | 8 (16/head) | 512 |
| 目标编码器 | 128 | 4 | 8 (16/head) | 512 |

### 编码维度分配 (D=128)

| 编码类型 | 维度范围 | 大小 | 可学习 |
|----------|---------|------|--------|
| Channel Embedding | [0, 32) | 32 | 是 |
| Temporal Position | [32, 64) | 32 | 否 (sincos) |
| Month Embedding | [64, 96) | 32 | 否 (sincos) |
| Spatial Embedding | [96, 128) | 32 | 否 (sincos+GSD) |
